// Handles keyboard, mouse, and touch joystick input.
// Exposes: axis {x,y} (normalized -1..1), aim (radians), mode.
...
export const Input = {
  ...
  axis: { x: 0, y: 0 },          // movement direction (normalized)
  joystickActive: false,
  joystickVec: { x: 0, y: 0 },
  mode: 'wasd',                   // 'wasd' | 'joystick' | 'both'
  ...
  _applyModeVisibility() {
    ...
    if (!zone) return;
    if (this.mode === 'joystick' || this.mode === 'both') {
  ...
  _initJoystick() {
    ...
    const R = 46; // max knob travel radius
    ...
    base.addEventListener('touchmove', e => {
      e.preventDefault();
      if (e.touches.length) updateFromTouch(e.touches[0]);
    }, { passive: false });
    ...
    window.addEventListener('mousemove', e => {
      if (mouseDown) updateFromTouch(e);
    });
  ...
  // Called each frame — computes axis vector from active sources
  update() {
    let x = 0, y = 0;
